...
Runner::Runner()
{
	if (!m_texture.loadFromFile("graphics/monster/monster_speed.png")) {
...
void Runner::initialize(int x1, int y1)
{
	frameNumber = 0;
	frameTimer = TIME_BETWEEN_2_FRAMES_PLAYER;
	frameMax = MAX_FRAME_MONSTER_SPEED;
	direction = LEFT;
	x = x1;
	y = y1;
	w = MONSTER_SPEED_WIDTH;
	h = MONSTER_SPEED_HEIGTH;
	onGround = 0;
	etat = 0;
}

int Runner::Update(Map & map, Player & player)
{
	if (life > 0)
	{
		dirX = 0;
...
void Runner::Draw(RenderWindow &window, Map &map)
{
	if (frameTimer <= 0)
	{
		frameTimer = TIME_BETWEEN_2_FRAMES_PLAYER;
